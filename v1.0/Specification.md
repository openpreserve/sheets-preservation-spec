...
| ODS_5 | Embedded objects are allowed in the ODF standard, and you may encounter these. Typically, the embedded object is an image file and very often so a logo image, but embedded objects may also be other spreadsheets, text documents, audio, video and even 3D objects.<br><br>If the embedded object has a file format, your organisation accepts, this policy will validate succesfully, but if not the embedded object should be converted to a file format, your organisation accepts. Secondly, embedded objects can be removed from the spreadsheet and either preserved as indenpendent objects or deleted entirely with an appraised acceptance of the data loss.<br><br>However, if you decide to remove any embedded objects from the spreadsheet, you should document the object and its relationship to the spreadsheet. This metadata can be saved in a sidecar file or associated table. The metadata must include the cell range which the embedded object covered. |
...
| ODS_7 | Macros contain programming code that can execute and manipulate data in the spreadsheet and the file system. These may present security risks, and it is therefore prudent to remove or disable any macros. Macros add interaction to a spreadsheet and can change data.<br><br>Removing macros will prevent any data from changing, but the macro itself will be lost if not documented other places.<br><br>Another option is to set up your spreadsheet rendering application to disable any macros. This will prevent any data from changing, it will preserve the interactivity of the macro and the macro may be reused by a user to support original reuse scenarios, but the same interactivity may also lead to change of data in unintended and non-transparent ways, if the user is not careful. |
